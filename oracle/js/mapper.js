...
(function checkIfElemExists() {
  if (!document.querySelector('.object-frame .col11')) {
    ...
    setTimeout(function() {
      ...
      attributes_dict = {
        "Target Similarity": similarities,
        "Distance to Object": distances,
        "Monthly Price": prices,
        "Area Price": pricesSqm,
      ...
      for (i=0; i<latitudes.length; i++) {
        ...
        for (var attribute in attributes_dict) {
          if (['Is Active', 'Is Online', 'Web Link'].indexOf(attribute) < 0) {
            subAttributes.push(["<strong>", attribute, "</strong>: ", attributes_dict[attribute][i], "<br>"].join(''));
          } else if (attribute == 'Is Online' && attributes_dict[attribute][i] == 'True') {
            subAttributes.push([
          ...
          } else if (attribute == 'Is Active' && attributes_dict[attribute][i] == 'True') {
            subAttributes.push([
          ...
          } else if (attribute == 'Is Active' && attributes_dict[attribute][i] == 'False') {
            subAttributes.push([
              "<div class='active-signal'>",
      ...
      var objectAddress = $('.apartment-location').find('.python-results-text').text().trim().replace('Object: ', ''),
      ...
      var buildMap = function(transport_data, plz_data) {
        ...
        var control_panel_layers = {
          "Bus Lines": bus_layers,
          "U-Bahn Lines": ubahn_layers,
          "S-Bahn Lines": sbahn_layers,
          "Tram Lines": tram_layers,
          "Postcode Polygons": plz_layers,
          "Street View": streets,
          "Monochrome View": grayscale
        };
